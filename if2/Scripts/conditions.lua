...
-- Modify testcond to support "on not X" etc. Vanilla never supported this because it couldn't parse it anyway.
									...
									for g=-1,1 do
										for h=-1,1 do
											if (b ~= "empty") then
												...
												if (unitmap[tileid] ~= nil) then
													for c,d in ipairs(unitmap[tileid]) do
														if d ~= unitid then -- Whoops, Hempuli forgot this test originally...
															local unit = mmf.newObject(d)
															if (nameMatches(getname(unit), b)) then
																result = false
															end
