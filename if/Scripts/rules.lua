			...
			if (donefirstwords[tileid] == nil) or ((donefirstwords[tileid] ~= nil) and (donefirstwords[tileid][dir] == nil)) and (limiter < 10000) then
							...
							if (variantcount[wordid] > 0) then
								...
								if (tiletype ~= 5) then
									...
									elseif (stage == 5) then
										if (wordid ~= #sentences) then
											...
											elseif (tiletype == 6) then
												prevstage = stage
												stage = 4
											elseif (tiletype == 10) and stage2reached then
												prevstage = stage
												stage = 10
											elseif (tiletype ~= 4) then
												prevstage = stage
									...
									elseif (stage == 10) then
										if (tilename == "all" or tiletype == 11) then
											prevstage = stage
											stage = 11
									...
									elseif (stage == 13) then
										if (tiletype == 3 or tiletype == 2) then
											prevstage = stage
											stage = 14
									...
									elseif (stage == 14) then
										if (tiletype == 6 or tiletype == 10) then
											prevstage = stage
											stage = 10
										else
											prevstage = stage
				...
				if (#finals > 0) then
					for i,sentence in ipairs(finals) do
						...
						local onto_ifconds = false
						local current_ifcond_quantifier = ''
						local current_ifcond_noun = nil
						local current_ifcond_cond = nil
						local current_ifcond_ids_if_valid = {}
						local group_ifconds = {}
						...
						if valid then
							for index,wdata in ipairs(sentence) do
								...
								if (onto_ifconds) then
									table.insert(current_ifcond_ids_if_valid, {wname, wtype, wid})
									if (wname == "all" and current_ifcond_quantifier == "" and current_ifcond_cond == nil) or wtype == 11 then
										current_ifcond_quantifier = prefix .. wname
									elseif wtype == 0 and current_ifcond_cond == nil then
										current_ifcond_noun = prefix .. wname
									elseif wtype == 2 and current_ifcond_noun ~= nil and current_ifcond_cond == nil then
										table.insert(group_ifconds, {current_ifcond_quantifier, current_ifcond_noun, {{prefix .. "cg5with", {wname}}}})
										current_ifcond_quantifier = ""
										current_ifcond_noun = nil
										for _,extraids_valid in ipairs(current_ifcond_ids_if_valid) do
											table.insert(extraids, {extraids_valid[1], extraids_valid[2], extraids_valid[3]})
										end
										current_ifcond_ids_if_valid = {}
									elseif wtype == 3 then
										table.insert(group_ifconds, {current_ifcond_quantifier, current_ifcond_noun, {{prefix .. wname, {}}}})
										current_ifcond_quantifier = ""
										current_ifcond_noun = nil
										for _,extraids_valid in ipairs(current_ifcond_ids_if_valid) do
											table.insert(extraids, {extraids_valid[1], extraids_valid[2], extraids_valid[3]})
										...
										current_ifcond_ids_if_valid = {}
									elseif wtype == 7 then
										current_ifcond_cond = prefix .. wname
									elseif wtype == 0 and current_ifcond_cond ~= nil then
										table.insert(group_ifconds, {current_ifcond_quantifier, current_ifcond_noun, {{current_ifcond_cond, {wname}}}})
										current_ifcond_quantifier = ""
										current_ifcond_noun = nil
										current_ifcond_cond = nil
										for _,extraids_valid in ipairs(current_ifcond_ids_if_valid) do
											table.insert(extraids, {extraids_valid[1], extraids_valid[2], extraids_valid[3]})
										end
										current_ifcond_ids_if_valid = {}
							...
							if #group_ifconds > 0 then
								table.insert(conds, {"if", group_ifconds})
							end
			...
			for i,cond in ipairs(conds) do
				if (cond[1] == "if" or cond[1] == "not if") then
					for _,ifcond in ipairs(cond[2]) do
						for _,innercond in ipairs(ifcond[3]) do
