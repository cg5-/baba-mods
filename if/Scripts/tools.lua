...
function writerules(parent,name,x_,y_)
	...
	if (texthide == nil) then
		for i,rules in ipairs(visualfeatures) do
			...
			if (#conds > 0) then
				for a,cond in ipairs(conds) do
					...
					if cond[1] == "if" or cond[1] == "not if" then
						ifcondtext = ifcondtext .. " " .. cond[1] .. " "
						for i,ifcond in ipairs(cond[2]) do
							if ifcond[1] ~= "" then
								ifcondtext = ifcondtext .. ifcond[1] .. " "
							end
							ifcondtext = ifcondtext .. ifcond[2] .. " is "

							innerconds = ifcond[3]
							for j, innercond in ipairs(ifcond[3]) do
								ifcondtext = ifcondtext .. condtostring(innercond, true)
								if j < #innerconds then
									ifcondtext = ifcondtext .. "& "
								end
							end
						...
						if middlecond then
							text = text .. condtostring(cond, false)
			...
			text = text .. rule[2] .. " " .. rule[3] .. ifcondtext
