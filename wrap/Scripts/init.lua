...
function mod.getadjacenttile(unitid,x,y,dir,warps)
	...
	if hasfeature(getname(unit),"is","wrap",unitid) ~= nil then
		if rx > roomsizex - 2 then
			rx = rx - (roomsizex - 2)
			table.insert(warps, {roomsizex - 1, y, 0, ry})
		elseif rx < 1 then
			rx = rx + (roomsizex - 2)
			table.insert(warps, {0, y, roomsizex - 1, ry})
		elseif ry > roomsizey - 2 then
			ry = ry - (roomsizey - 2)
			table.insert(warps, {x, roomsizey - 1, rx, 0})
		elseif ry < 1 then
			ry = ry + (roomsizey - 2)
			table.insert(warps, {x, 0, rx, roomsizey - 1})
		end
	end
