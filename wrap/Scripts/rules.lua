		...
		while not done do
			...
			for idx, summary in ipairs(codeaffectingfeaturessummaries) do
				if summary == newsummary then
					...
					-- we have convergence, or it's equal to some previous summarywe already went through, in which case
...
local function addbacktofirstwords(firstwords, unitid, dir)
								...
								if (tiletype == 5) then
									...
									if secondaryfound then
										--if (string.len(tilename) == 1) then
											...
											addbacktofirstwords(firstwords, prevdata[3], prevdata[4])
										--else
									...
									if (wordid < #sentences) then
										...
										elseif (wordid == 1) and (blockfirstwords == false) then
											if (nexts[3] ~= -1) then
												...
												addbacktofirstwords(firstwords, nexts[3], nexts[4])
											end
...
function codecheck(unitid, x, y, dir)
	...
	return gettextattile(rx, ry), rx, ry, rdir, warps
end
					...
					if (#cond[2] > 0) then
						...
						if allfound then
							for a,mat in pairs(objectlist) do
								if (alreadyused[a] == nil) and (a ~= "group") and (a ~= "all") and (a ~= "text") and (a ~= "paradoxmessage") then
