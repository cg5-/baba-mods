	...
	if (conds ~= nil) then
		if (#conds > 0) then
			...
			for i,cond in ipairs(conds) do
				...
				-- THOSE START
				elseif (condtype == "those" or condtype == "not those") then
					valid = true
					local ourresult = true

					local word = mmf.newObject(wordid)
					...
					if (ox == 0) then
						ourresult = ourresult and (x == wx)
					else
						ourresult = ourresult and (ox * x > ox * wx)
					end

					if (oy == 0) then
						ourresult = ourresult and (y == wy)
					else
						ourresult = ourresult and (oy * y > oy * wy)
					end
					
					if (condtype == "not those") then
